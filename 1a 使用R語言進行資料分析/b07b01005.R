...
##############################################################################
library(plyr)
ddply(iris, .variables = .(Species), .fun = summarise, avg_PW = mean(Petal.Width, na.rm = TRUE))
#mean(subset(iris, subset = Species=='setosa')$Petal.Width, na.rm = TRUE)
#mean(subset(iris, subset = Species=='versicolor')$Petal.Width, na.rm = TRUE)
#mean(subset(iris, subset = Species=='virginica')$Petal.Width, na.rm = TRUE)
#unique(iris$Species)
...
#c(1:100)[is.prime(1:100)]
#c(1:nrow(iris))[is.prime(1:nrow(iris))]

iris$Petal.Width[is.prime(1:nrow(iris))]
